...
class HeatmiserThermostat(object):
    ...
    def get_model(self, refresh=False):
        if self.dcb == "" or refresh:
            self.read_dcb()
        return self.dcb[4]["value"]

    def get_frost_temp(self, refresh=False):
        """
        Returns the temperature
        """
        if self.dcb == "" or refresh:
            self.read_dcb()
        return self.dcb[17]["value"]

    def get_target_temp(self, refresh=False):
        """
        Returns the temperature
        """
        if self.dcb == "" or refresh:
            self.read_dcb() 
        return self.dcb[18]["value"]

    def get_floormax_temp(self, refresh=False):
        """
        Returns the temperature
        """
        if self.dcb == "" or refresh:
            self.read_dcb() 
        return self.dcb[19]["value"]

    def get_status(self, refresh=False):         
        if self.dcb == "" or refresh:
            self.read_dcb() 
        return self.dcb[21]["value"]

    def get_heating(self, refresh=False):
        if self.dcb == "" or refresh:
            self.read_dcb()
        return self.dcb[23]["value"]

    def get_thermostat_id(self, refresh=False):
        if self.dcb == "" or refresh:
            self.read_dcb()
        return self.dcb[11]["value"]

    def get_temperature_format(self, refresh=False):
        if self.dcb == "" or refresh:
            self.read_dcb()
    ...
    def get_sensor_selection(self, refresh=False):
        if self.dcb == "" or refresh:
            self.read_dcb()
    ...
    def get_program_mode(self, refresh=False):
        if self.dcb == "" or refresh:
            self.read_dcb()
    ...
    def get_frost_protection(self, refresh=False):
        if self.dcb == "" or refresh:
            self.read_dcb()
        return self.dcb[7]["value"]

    def get_floor_temp(self, refresh=False):
        if self.dcb == "" or refresh:
            self.read_dcb()
        return (
    ...
    def get_sensor_error(self, refresh=False):
        if self.dcb == "" or refresh:
            self.read_dcb()
        return self.dcb[34]["value"]

    def get_current_state(self, refresh=False):
        if self.dcb == "" or refresh:
            self.read_dcb() 
    ...
    def parse_temperature(self, high_byte: int, low_byte: int) -> float | None:
        """
        Parse Heatmiser temperature from high/low bytes according to the DCB spec.
        
        Args:
            high_byte (int): High 8 bits (0-255)
            low_byte (int): Low 8 bits (0-255)
            
        Returns:
            float | None: Temperature in °C (with one decimal), or None if sensor not connected.
        """
    ...
    def get_air_temp(self,refresh=False):
        if self.dcb == "" or refresh:
            self.read_dcb()
        return self.parse_temperature(self.dcb[32]["value"], self.dcb[33]["value"])   
    
    def get_floor_temp(self,refresh=False):
        if self.dcb == "" or refresh:
            self.read_dcb()
        return self.parse_temperature(self.dcb[30]["value"], self.dcb[31]["value"])
    
    def get_remote_temp(self,refresh=False):
        if self.dcb == "" or refresh:
            self.read_dcb()
        return self.parse_temperature(self.dcb[28]["value"], self.dcb[29]["value"])
    
    def get_run_mode(self, refresh=False):
        if self.dcb == "" or refresh:
            self.read_dcb() 
        modes = {
            0: "heat",
        ...
        return modes.get(self.dcb[23]["value"], "Unknown")
    
    def get_hotwater_state(self, refresh=False):
        if self.dcb == "" or refresh:
            self.read_dcb()
        states = {
            0: "Off",
        ...
        return states.get(self.dcb[36]["value"], "Unknown")
    

    class HotWaterWriteState(IntEnum):
        TIMED = 0
        ON = 1
        OFF = 2

    def set_hotwater_state(self, state: HotWaterWriteState) -> bool:        
        self._hm_send_address(self.address, 42, state.value, 1)
        return True
